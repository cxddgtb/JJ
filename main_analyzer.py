...
def calculate_indicator_pressure_support(df):
    ...
    # 趋势与超卖确认买点
    A3 = (C - LLV(L, 9)) / (HHV(H, 9) - LLV(L, 9)).replace(0, np.finfo(float).eps) * 100
    A4 = SMA(A3, 3, 1)
    A5 = SMA(A4, 3, 1)
    
    ROC_C = C - REF(C, 1)
    ABS_ROC_C = ROC_C.abs()
    EMA_ROC_C_6 = EMA(ROC_C, 6)
    EMA_ABS_ROC_C_6 = EMA(ABS_ROC_C, 6)
    
    # 避免分母为0
    denom_A8 = EMA_ABS_ROC_C_6
    denom_A8 = denom_A8.replace(0, np.finfo(float).eps)
    A8 = 100 * EMA(EMA_ROC_C_6, 6) / denom_A8

    # 注意：COUNT(A8<0,2) > 0 表示在最近2个周期中至少有1次A8<0
    buy_condition_A8_count = (COUNT(A8 < 0, 2) >= 1)
    买 = (LLV(A8, 2) == LLV(A8, 7)) & buy_condition_A8_count & CROSS(A8, SMA(A8, 2, 1))

    # 黄金线 (此指标不用于买卖信号判断，但可作为参考)
    # 黄金线 = EMA(C, 18)

    # 结合买卖信号
    buy_signal = CROSS(C, 买入) | ((L < 买入) & (C > 买入)) | (买 == True)
    sell_signal = CROSS(卖出, C) | ((H > 卖出) & (C < 卖出))

    # 返回最新的信号
    return {
        'buy_signal_ps': buy_signal.iloc[-1],
        'sell_signal_ps': sell_signal.iloc[-1]
    }
...
def calculate_indicator_chip_intent(df):
    ...
    # VAR1 = HHV(H, 3)
    # VAR2 = (WINNER_PLACEHOLDER(1.1 * C) - WINNER_PLACEHOLDER(0.9 * C)) * 100
    # VAR3 = VAR1 # VAR1不会是负数
    # VAR4 = VAR2.apply(lambda x: 0 if x < 0 else x)
    # 震仓 = PWINNER_PLACEHOLDER(10, VAR4) * 100 # 无法准确实现
    ...
    # IF(CLOSE*1.3,VAR3Q*10,VAR3Q/10) - 假设 CLOSE*1.3 总是真 (即CLOSE>0)
    VAR4Q = EMA(VAR3Q * 10, 3) 

    VAR5Q = LLV(L, 30)
    VAR6Q = HHV(VAR4Q, 30)
    
    # VAR7Q: MA(CLOSE,58)如果有效则为1，否则为0。这里假设数据充足，MA有效。
    VAR7Q = EMA(C, 58).notna().astype(int) 

    # IF(L<=VAR5Q,(VAR4Q+VAR6Q*2)/2,0)
    term_if_true = (VAR4Q + VAR6Q * 2) / 2
    VAR8Q_numerator = (L <= VAR5Q).astype(int) * term_if_true
    VAR8Q = EMA(VAR8Q_numerator, 3) / 618 * VAR7Q # 618作为分母

    VAR9Q = VAR8Q.apply(lambda x: min(x, 100)) # IF(VAR8Q>100,100,VAR8Q)
    吸筹 = VAR9Q > 0 
    ...
    AA5 = SMA(AA4, 13, 8) # M=8, EMA-like
    走势 = np.ceil(SMA(AA5, 13, 8)) # M=8, EMA-like
    AA6 = SMA(AA3, 21, 8) # M=8, EMA-like
    卖临界 = (走势 - AA6) > 85

    # 结合买卖信号
    # 注意：吸筹信号通常是正向指标，但这里没有明确的买入交叉点，
    # 假定吸筹柱出现 (VAR9Q>0) 就可以认为是一个积极信号。
    buy_signal = 买入信号.iloc[-1] | 吸筹.iloc[-1]
    sell_signal = 卖出信号.iloc[-1] | 卖临界.iloc[-1]
...
def calculate_indicator_main_force(df):
    ...
    VAR5 = EMA(VAR5_numerator, 3)

    主进 = VAR5 > REF(VAR5, 1)
    # 洗盘 = VAR5 < REF(VAR5, 1) # 洗盘通常不是一个买入信号，而是震荡信号
    ...
    金山 = EMA((L <= LLV(L, 30)).astype(int) * VARC, 3) # 金山>0意味着可能在底部

    # 结合买卖信号
    buy_signal = 主进.iloc[-1] | 波段介入点.iloc[-1] | (金山.iloc[-1] > 0)
    sell_signal = 主出.iloc[-1] | 冲顶.iloc[-1]
...
def get_index_name(index_code):
    """
    根据指数代码获取名称。AkShare的基金或股票接口可能需要不同的方式。
    这里尝试几种方式，并包含一个简单的硬编码映射。
    """
    try:
        if '.' in index_code: # 带市场后缀，可能是股票或特定指数
            symbol = index_code.split('.')[0]
            if index_code.endswith('.SH') or index_code.endswith('.SZ'):
                # 尝试获取A股股票名称 (包括ETF，虽然它不是严格意义上的'股票')
                stock_info_df = ak.stock_info_a_code_name()
                name_match = stock_info_df[stock_info_df['code'] == symbol]['name']
                if not name_match.empty:
                    return name_match.iloc[0]
            # 对于纯指数，尝试通过代码直接查找
            if symbol in ['000001', '399001', '000300', '000905']: # 常见指数
                 index_names = {
                    '000001': '上证指数',
                    '399001': '深证成指',
                    '000300': '沪深300',
                 ...
                 return index_names.get(symbol, f"未知_{symbol}")
        
        # 尝试通过基金接口获取ETF名称 (AkShare的fund_etf_spot_em有时可以提供名称)
        fund_list = ak.fund_etf_spot_em()
        name_df = fund_list[fund_list['代码'] == index_code]
        if not name_df.empty:
            return name_df['名称'].iloc[0]
        
        # 默认硬编码映射 (如果AkShare查找失败)
        mapping = {
            '000001.SH': '上证指数',
            '399001.SZ': '深证成指',
            '000300.SH': '沪深300',
...
def get_historical_data(index_code):
    """
    从AkShare获取指数/ETF历史数据。
    """
    try:
        df = None
        # 尝试获取指数数据 (例如 000001.SH, 399001.SZ)
        if index_code.endswith('.SH') or index_code.endswith('.SZ'):
            symbol_without_market = index_code.split('.')[0]
            if symbol_without_market.startswith(('0', '3', '8', '9')): # 常见指数代码开头
                df = ak.stock_zh_index_daily(symbol=symbol_without_market)
            else: # 可能是ETF，尝试基金接口
                df = ak.fund_etf_hist_em(symbol=index_code) # ETF代码通常不带后缀或AkShare直接支持
        else: # 尝试作为ETF代码处理
             df = ak.fund_etf_hist_em(symbol=index_code)

        if df is None or df.empty:
            print(f"AkShare未能获取 {index_code} 的数据，尝试其他方式或检查代码。")
            return None
        ...
        df = df.sort_index()
        return df[['open', 'close', 'high', 'low']] # 只保留OHLC，不包括成交量和成交额，除非指标需要
...
def analyze_index(index_code, repo_root):
    ...
    required_data_points = max(58, 33, 25) + 5 # 留一些余量
    ...
    row_data = [
        ...
        "TRUE" if results_ci['sell_signal_ci'] else "FALSE",
